...
namespace karst {
    ...
    struct PrintingConfig {
        ...
        bool do_save_txt           {true};     ///< if true data about network (nodes, pores and grains) is saved in text file
        bool do_save_table         {true};     ///< if true save tables with diameters, flow and concentration
        bool do_save_topology      {true}; 	   ///< if true topology is saved in each save_all
        bool do_verbose            {false};    ///< if true verbose version for debugging
        bool do_debugging_printing {false};	   ///< if true debugging printing is done after each calculation
