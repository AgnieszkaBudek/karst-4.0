...
// A struct to store the triangulation results
    ...
    TriangulationResult2D generate_and_triangulate(int Nx, int Ny, TriangulationResult2D& result, Network& S) {
        if (Nx <= 0 || Ny <= 0) {
...
//                    //idx_by_point.emplace(q, i); // każdy kafel wskazuje na indeks w komórce podstawowej
//                }

        // 2. Create the triangulation
        Triangulation_2 dt;
        dt.insert(result.points.begin(), result.points.end());
        ...
        for (int i = 0; i < static_cast<int>(result.points.size()); ++i) {
            idx_by_point.emplace(result.points[i], i);
        }

        // 3. Extract edges
        const auto n_edges = static_cast<size_t>(std::distance(dt.finite_edges_begin(), dt.finite_edges_end()));
        ...
        for (Triangulation_2::Finite_edges_iterator eit = dt.finite_edges_begin(); eit != dt.finite_edges_end(); ++eit) {
            Triangulation_2::Face_handle face = eit->first;
            int vertex_index = eit->second;
            Point_2 p1 = face->vertex(Triangulation_2::Face::cw(vertex_index))->point();
            Point_2 p2 = face->vertex(Triangulation_2::Face::ccw(vertex_index))->point();
            int i = idx_by_point.at(p1);
            int j = idx_by_point.at(p2);
            result.edges.emplace_back(i, j);
        }

        // 4. Extract faces (triangles)
        result.faces.reserve(dt.number_of_faces());
        for (Triangulation_2::Finite_faces_iterator fit = dt.finite_faces_begin(); fit != dt.finite_faces_end(); ++fit) {
            ...
            result.faces.push_back({i0, i1, i2});
        }
